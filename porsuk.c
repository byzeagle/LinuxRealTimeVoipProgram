...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "hlvc:p:b:")) != -1) {
		char *pc;
		switch (opt) {
		case 'h':
			help_use();
	...
	// Check inputs
	if ((call && listen) || (!call && !listen)) {
		fprintf(stderr, "either -c hostname[:port] or -l have to be called\n");
		exit(EXIT_FAILURE);
	}
	...
	if (listen) {
		...
		char buffer[6];
		if ((bytesNread = recvfrom(sockfd, buffer,
				sizeof(buffer), 0, (struct sockaddr*) &remote_addr,
				&remote_addr_len)) < 0) {
			perror("recvfrom()");
			exit(EXIT_FAILURE);
		}
	...
	if (call) {
		...
		if ((bytesNsent = sendto(sockfd, "START", strlen("START"), 0,
